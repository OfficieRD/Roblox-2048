...
local VolMusic = 0.5 -- Volumen por defecto Música
				...
				while nr >= 1 and nr <= BOARD_SIZE and nc >= 1 and nc <= BOARD_SIZE do
					...
					elseif newBoard[nr][nc] == val and not mergedFlags[nr][nc] then
						...
						if GAME_MODE == "Special" then
							-- NUEVA LÓGICA: Solo da diamantes al crear fichas altas
							local tileCreated = val * 2 -- 'val' es la ficha que se fusionó (ej: 16), tileCreated es el resultado (32)
							...
							if tileCreated >= 32 then
								...
								if reward > 0 then
									...
									-- Efecto visual opcional: Sonido especial o brillo aquí
								end
							end
						else
...
local function checkIsUnlocked(data)
	...
	if data.IsVIP then
		-- A. Revisar memoria local primero (Instantáneo)
		if localSessionPasses[VIP_GAMEPASS_ID] == true then return true end

		-- B. Si no está en memoria, preguntar a Roblox
...
local function updateRightPanel(data)
	...
	if isUnlocked then
		...
		elseif data.ReqAttribute then
			-- ? CORRECCIÓN: Si es un título de Season, mostrar su descripción, no buscar un Score que no existe
			DescLabel.Text = data.Desc .. " (Locked)"
		else 
			-- Solo entra aquí si tiene un requisito de Score numérico
