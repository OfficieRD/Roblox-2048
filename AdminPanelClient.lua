	...
	MainFrame.Size = UDim2.new(0, 450, 0, 600) -- MÁS GRANDE
	...
	createLabel("4. SPECIALS", 11) -- Ajustamos orden
	local AttrBox = createInput("Attribute Name (e.g. Title_S1_HS_1)", 11)

	createBtn("GIVE ATTRIBUTE ??", Color3.fromRGB(160, 50, 255), 12, function()
		AdminEvent:FireServer("GiveAttribute", TargetBox.Text, AttrBox.Text)
	end)

	createBtn("UNLOCK ALL SKINS/TITLES ??", Color3.fromRGB(255, 0, 255), 13, function()
		AdminEvent:FireServer("UnlockAllTitles", TargetBox.Text, nil)
	end)

-- Botón nuevo para Resetear Nivel
createBtn("RESET LEVEL ONLY ??", Color3.fromRGB(255, 100, 50), 15, function()
	AdminEvent:FireServer("ResetLevel", TargetBox.Text, nil)
end)

-- CONTROLES DE RACHA
createBtn("SET STREAK DAYS ??", Color3.fromRGB(255, 140, 0), 16, function()
	AdminEvent:FireServer("SetStreak", TargetBox.Text, AmountBox.Text)
end)

createBtn("BREAK STREAK ??", Color3.fromRGB(100, 100, 100), 17, function()
